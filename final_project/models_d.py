...
class GlobalDiscriminator(nn.Module):
    ...
    def __init__(self, input_shape):
        ...
        # input_shape: (None, 128, img_h//4, img_w//4)
        self.offset3 = ConvOffset2D(128)
        self.conv3 = nn.Conv2d(128, 256, kernel_size=5, stride=2, padding=2)
        self.bn3 = nn.BatchNorm2d(256)
        self.act3 = nn.ReLU()

        # input_shape: (None, 256, img_h//8, img_w//8)
        self.offset4 = ConvOffset2D(256)
        self.conv4 = nn.Conv2d(256, 512, kernel_size=5, stride=2, padding=2)
        self.bn4 = nn.BatchNorm2d(512)
        self.act4 = nn.ReLU()

        # input_shape: (None, 512, img_h//16, img_w//16)
        self.offset5 = ConvOffset2D(512)
        self.conv5 = nn.Conv2d(512, 512, kernel_size=5, stride=2, padding=2)
        self.bn5 = nn.BatchNorm2d(512)
        self.act5 = nn.ReLU()
        # input_shape: (None, 512, img_h//32, img_w//32)
    ...
    def forward(self, x):
        x = self.bn1(self.act1(self.conv1(x)))
        x = self.bn2(self.act2(self.conv2(x)))
        x = self.bn3(self.act3(self.conv3(self.offset3(x))))
        x = self.bn4(self.act4(self.conv4(self.offset4(x))))
        x = self.bn5(self.act5(self.conv5(self.offset5(x))))
        x = self.act6(self.linear6(self.flatten6(x)))
        return x
