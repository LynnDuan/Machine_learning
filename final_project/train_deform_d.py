...
from models_d import CompletionNetwork, ContextDiscriminator
...
parser.add_argument('--steps_1', type=int, default=9000)#default=90000)
parser.add_argument('--steps_2', type=int, default=6000)#default=10000)
parser.add_argument('--steps_3', type=int, default=400000)#default=400000)
parser.add_argument('--snaperiod_1', type=int, default=1800)#default=18000)
parser.add_argument('--snaperiod_2', type=int, default=2000)#default=2000)
parser.add_argument('--snaperiod_3', type=int, default=8000)#default=80000)
...
def main(args):
    ...
    if load_pretained:
        print('load pretrained model', args.model)
        model_cn.load_state_dict(torch.load(args.model, map_location='cpu'))
    ...
    if not load_pretained:
        iter1 = 0
        while iter1 < args.steps_1:
            for x in train_loader:
                ...
                if iter1 % args.snaperiod_1 == 0:
                    with torch.no_grad():
                        ...
                        save_image(imgs, os.path.join(args.result_dir, 'phase_1', 'step%d.png' % iter1), nrow=len(x))
                        torch.save(model_cn.state_dict(), os.path.join(args.result_dir, 'phase_1', 'model_cn_step%d' % iter1))

                # if pbar.n >= args.steps_1:
                if iter1 >= args.steps_1:
                    break
        # pbar.close()
        ...
        for x in train_loader:
            ...
            if iter2 % args.snaperiod_2 == 0:
                with torch.no_grad():
                    ...
                    save_image(imgs, os.path.join(args.result_dir, 'phase_2', 'step%d.png' % iter2), nrow=len(x))
                    torch.save(model_cd.state_dict(), os.path.join(args.result_dir, 'phase_2', 'model_cd_step%d' % iter2))

            # if pbar.n >= args.steps_2:
            if iter2 >= args.steps_2:
                break
    # pbar.close()
        ...
        for x in train_loader:
            ...
            if iter3 % args.snaperiod_3 == 0:
                with torch.no_grad():
                    ...
                    save_image(imgs, os.path.join(args.result_dir, 'phase_3', 'step%d.png' % iter3), nrow=len(x))
                    torch.save(model_cn.state_dict(), os.path.join(args.result_dir, 'phase_3', 'model_cn_step%d' % iter3))
                    torch.save(model_cd.state_dict(), os.path.join(args.result_dir, 'phase_3', 'model_cd_step%d' % iter3))

            # if pbar.n >= args.steps_3:
            if iter3 >= args.steps_3:
                break
    # pbar.close()
